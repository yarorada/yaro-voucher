...
export default function Accounting() {
  ...
  const handleShare = async () => {
    const existing = (existingShares as any[]).find((s) => s.year === year && s.month === month);
    let token: string;
    if (existing) {
      ...
      const { data, error } = await supabase
        ...
        .select()
        .single();
      if (error) { toast.error("Chyba při vytváření odkazu"); return; }
      token = (data as any).share_token;
      queryClient.invalidateQueries({ queryKey: ["accounting-shares"] });
    }
    const url = `${window.location.origin}/accounting/share/${token}`;
  ...
  usePageToolbar(
    <div className="flex items-center gap-1.5 w-full min-w-0">
      ...
      <Button variant="outline" size="sm" className="hidden sm:inline-flex h-8 text-xs gap-1 shrink-0" onClick={handleShare}>
