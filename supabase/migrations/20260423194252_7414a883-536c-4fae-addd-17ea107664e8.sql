-- Wallet auto earning: process completed deals and credit wallet points
CREATE OR REPLACE FUNCTION public.process_wallet_earnings(_deal_id uuid DEFAULT NULL)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    IF v_net_revenue <= 0 THEN
      -- Mark as processed even if no points earned
      UPDATE public.deals
    ...
    IF v_points > 0 THEN
      ...
      VALUES
        (v_deal.lead_client_id, v_deal.id, 'earn', v_points,
         'Automaticky přičteno za dokončený obchod', v_deal.user_id);
    END IF;
...
-- Schedule daily job at 03:00 UTC if pg_cron is available
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    -- Unschedule existing job if any (idempotent)
    IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'process_wallet_earnings_daily') THEN
      PERFORM cron.unschedule('process_wallet_earnings_daily');
    END IF;

    PERFORM cron.schedule(
      'process_wallet_earnings_daily',
      '0 3 * * *',
      $cron$ SELECT public.process_wallet_earnings(NULL); $cron$
    );
  END IF;
END $$;