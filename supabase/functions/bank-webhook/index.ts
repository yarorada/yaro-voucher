...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Accept various formats from Zapier/Make
    // Zapier typically sends: { body_plain, subject, from, ... }
    // Make might send: { text, subject, ... }
    // Or user can configure custom JSON: { emailText }
    ...
    if (!emailText || typeof emailText !== 'string' || emailText.trim().length < 10) {
      return new Response(JSON.stringify({ error: 'No email text found in payload', received_keys: Object.keys(body) }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
